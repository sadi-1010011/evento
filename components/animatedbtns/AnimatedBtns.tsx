...
export function FavoritedBtn({ checked, onclick }: { checked: boolean, onclick: Function }) {
    return (
        <div title="Like" className={styles.heart_container}>
            <input id="Give-It-An-Id" className={styles.checkbox} type="checkbox" checked={ checked } onChange={ ()=> onclick(!checked) } />
...
export function RemindMeBtn() {
    const [checked, setCheck] = useState(false);
    return (
    <label className={styles.container}>
        <input onClick={ ()=> { setCheck(!checked); checked && alert('event reminder coming soon!'); }} type="checkbox" defaultChecked={checked} />
        ...
        <svg className={styles.bell_solid} xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 448 512">
