...
export default function CatogoryCard({ active }: { active: string }) {
    ...
    return (
        catogories.map(catogory =>
            <div key={catogory} onClick={ (e)=> setSelectedCatogory(e.currentTarget?.lastChild?.textContent ? (e.currentTarget.lastChild.textContent).toLowerCase() : '' ) /* router.push('/home') */ } className={`w-1/4 p-1 inline-block m-2 rounded-xl my-2 ${selectedCatogory === catogory ? 'bg-green-300 text-black' : 'bg-green-100 text-black'} `}>
