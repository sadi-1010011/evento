...
export default function FavoritesCard({ title }: { title: string }) {
    ...
    return (
        <div id="favcard" ref={favcardref} className="flex w-full items-center justify-between py-4 px-3 my-0.5 bg-slate-700 rounded-s-full" style={{ 'boxShadow': 'rgba(0, 0, 0, 0.25) 0px 5px 5px, rgba(0, 0, 0, 0.12) 0px -5px 10px, rgba(0, 0, 0, 0.12) 0px 5px 2px'}}>
            <Image className="rounded-s-md mr-3" priority src={DummyImg} width={80} height={60} alt="event thumbnail" />
            <div className="flex flex-col w-full px-2">
                <h1 className="text-md text-white font-bold capitalize m-0 p-0">{title}</h1>
